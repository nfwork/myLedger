...
3. **`BearerAuthFilter`**：无令牌或 JWT 无效/过期则 **401**：`{"success":false,"message":"未登录或访问令牌已过期"}`；通过后把 **`user_id`、`username`、`nickname`** 写入 **`HttpServletRequest` 的 attribute**。
...
curl -s -X POST http://127.0.0.1:8080/ledger_settings/account.query!listByUser ^
  -H "Content-Type: application/json" ^
